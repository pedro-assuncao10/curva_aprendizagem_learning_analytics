...
Uso (da raiz do projeto):
  python 05_kmeans_curvas_ciclo/kmeans_ciclos.py            # 4 bins -> 4bins/
  python 05_kmeans_curvas_ciclo/kmeans_ciclos.py --bins 8   # -> 8bins/
"""
...
BASE_DIR = os.path.dirname(PASTA_SCRIPT)          # raiz do projeto
DATA_DIR = os.path.join(BASE_DIR, "dataset")
OUT_DIR = os.path.join(PASTA_SCRIPT, "4bins")
CACHE = os.path.join(DATA_DIR, "cache_vle_fff2014j.csv")
...
def main():
    ...
    if args.out:
        OUT_DIR = (args.out if os.path.isabs(args.out)
                   else os.path.join(PASTA_SCRIPT, args.out))
    elif args.bins != 4:
        OUT_DIR = os.path.join(PASTA_SCRIPT, f"{args.bins}bins")
